...
fn test_movss_mem_to_xmm15() {
    let code = [
        0xf3, 0x44, 0x0f, 0x10, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ]; // MOVSS XMM15, [0x3000]; HLT
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movss_xmm15_to_mem() {
    let code = [
        0xf3, 0x44, 0x0f, 0x11, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ]; // MOVSS [0x3000], XMM15; HLT
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movsd_mem_to_xmm15() {
    let code = [
        0xf2, 0x44, 0x0f, 0x10, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ]; // MOVSD XMM15, [0x3000]; HLT
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movsd_xmm15_to_mem() {
    let code = [
        0xf2, 0x44, 0x0f, 0x11, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ]; // MOVSD [0x3000], XMM15; HLT
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
