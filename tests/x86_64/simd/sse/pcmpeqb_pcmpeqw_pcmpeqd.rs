...
fn test_pcmpeqb_all_different() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x6f, 0x0b, 0x66, 0x0f, 0x74, 0xc1, 0xf4,
    ]);
...
fn test_pcmpeqb_mixed() {
    ...
    let data2 = [
        0x01, 0xFF, 0x03, 0xFF, 0x05, 0xFF, 0x07, 0xFF, 0x09, 0xFF, 0x0B, 0xFF, 0x0D, 0xFF, 0x0F,
        0xFF,
    ];
...
fn test_pcmpeqb_from_memory() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x74, 0x03, // PCMPEQB XMM0, [RBX]
        0xf4,
    ]);
...
fn test_pcmpeqb_alternating() {
    ...
    let data2 = [
        0xAA, 0x00, 0xCC, 0x00, 0xEE, 0x00, 0x00, 0x00, 0x22, 0x00, 0x44, 0x00, 0x66, 0x00, 0x88,
        0x00,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pcmpeqw_all_equal() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x6f, 0x0b, 0x66, 0x0f, 0x75,
        0xc1, // PCMPEQW XMM0, XMM1
        0xf4,
    ]);
...
fn test_pcmpeqw_all_different() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x6f, 0x0b, 0x66, 0x0f, 0x75, 0xc1, 0xf4,
    ]);
...
fn test_pcmpeqw_mixed() {
    ...
    let data2 = [
        0x01, 0x00, 0xFF, 0xFF, 0x03, 0x00, 0xFF, 0xFF, 0x05, 0x00, 0xFF, 0xFF, 0x07, 0x00, 0xFF,
        0xFF,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pcmpeqw_from_memory() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x75, 0x03, // PCMPEQW XMM0, [RBX]
        0xf4,
    ]);
...
fn test_pcmpeqd_all_equal() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x6f, 0x0b, 0x66, 0x0f, 0x76,
        0xc1, // PCMPEQD XMM0, XMM1
        0xf4,
    ]);
...
fn test_pcmpeqd_all_different() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x6f, 0x0b, 0x66, 0x0f, 0x76, 0xc1, 0xf4,
    ]);
...
fn test_pcmpeqd_mixed() {
    ...
    let data2 = [
        0x01, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x03, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF,
        0xFF,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pcmpeqd_from_memory() {
    ...
    full_code.extend_from_slice(&ALIGNED_ADDR2.to_le_bytes());
    full_code.extend_from_slice(&[
        0x66, 0x0f, 0x6f, 0x00, 0x66, 0x0f, 0x76, 0x03, // PCMPEQD XMM0, [RBX]
        0xf4,
    ]);

    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = [
        0xEF, 0xCD, 0xAB, 0x90, 0xEF, 0xCD, 0xAB, 0x90, 0xEF, 0xCD, 0xAB, 0x90, 0xEF, 0xCD, 0xAB,
        0x90,
    ];
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR2)).unwrap();
...
fn test_pcmpeqd_negative_values() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = [
        0x00, 0x00, 0x00, 0x80, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x80, 0xFF, 0xFF, 0xFF,
        0xFF,
    ];
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR2)).unwrap();
...
fn test_pcmpeqw_xmm10_xmm11() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = [
        0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x12,
        0x34,
    ];
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR2)).unwrap();
