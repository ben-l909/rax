...
fn test_pmaxub_identity_operation() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = [
        0x42, 0x13, 0xAB, 0xCD, 0xEF, 0x55, 0x99, 0x77, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88,
    ];
...
fn test_pmaxuw_first_larger_all() {
    ...
    let data2 = [
        0x00, 0x10, 0x01, 0x10, 0x02, 0x10, 0x03, 0x10, 0x04, 0x10, 0x05, 0x10, 0x06, 0x10, 0x07,
        0x10,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pmaxuw_identity() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = [
        0x34, 0x12, 0x78, 0x56, 0xBC, 0x9A, 0xF0, 0xDE, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
        0x88,
    ];
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_pmaxuw_mixed_high_low() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data1 = [
        0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0xFF, 0xFF, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x12,
        0x34,
    ];
    let data2 = [
        0xFF, 0xFF, 0x00, 0x00, 0x80, 0x80, 0x80, 0x80, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
        0xCC,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pmaxuw_sequential_pattern() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data1 = [
        0x00, 0x00, 0x10, 0x00, 0x20, 0x00, 0x30, 0x00, 0x40, 0x00, 0x50, 0x00, 0x60, 0x00, 0x70,
        0x00,
    ];
    let data2 = [
        0x08, 0x00, 0x18, 0x00, 0x28, 0x00, 0x38, 0x00, 0x48, 0x00, 0x58, 0x00, 0x68, 0x00, 0x78,
        0x00,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
...
fn test_pmaxuw_inverse_patterns() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data1 = [
        0x00, 0x80, 0x01, 0x81, 0x02, 0x82, 0x03, 0x83, 0x04, 0x84, 0x05, 0x85, 0x06, 0x86, 0x07,
        0x87,
    ];
    let data2 = [
        0xFF, 0x7F, 0xFE, 0x7E, 0xFD, 0x7D, 0xFC, 0x7C, 0xFB, 0x7B, 0xFA, 0x7A, 0xF9, 0x79, 0xF8,
        0x78,
    ];
    mem.write_slice(&data1, GuestAddress(ALIGNED_ADDR)).unwrap();
    mem.write_slice(&data2, GuestAddress(ALIGNED_ADDR2))
