...
fn test_movhpd_mem_to_xmm15() {
    let code = [
        0x66, 0x44, 0x0f, 0x16, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movhpd_xmm15_to_mem() {
    let code = [
        0x66, 0x44, 0x0f, 0x17, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movlpd_mem_to_xmm15() {
    let code = [
        0x66, 0x44, 0x0f, 0x12, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_movlpd_xmm15_to_mem() {
    let code = [
        0x66, 0x44, 0x0f, 0x13, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
