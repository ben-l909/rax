...
fn test_aesimc_xmm0_mem() {
    let code = [
        0x66, 0x0f, 0x38, 0xdb, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_aesimc_xmm7_mem() {
    let code = [
        0x66, 0x0f, 0x38, 0xdb, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_aesimc_xmm15_mem() {
    let code = [
        0x66, 0x44, 0x0f, 0x38, 0xdb, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_aeskeygenassist_xmm0_mem_0x01() {
    let code = [
        0x66, 0x0f, 0x3a, 0xdf, 0x04, 0x25, 0x00, 0x30, 0x00, 0x00, 0x01, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
...
fn test_aeskeygenassist_xmm7_mem_0x0a() {
    let code = [
        0x66, 0x0f, 0x3a, 0xdf, 0x3c, 0x25, 0x00, 0x30, 0x00, 0x00, 0x0a, 0xf4,
    ];
    let (mut vcpu, _) = setup_vm(&code, None);
    run_until_hlt(&mut vcpu).unwrap();
}
