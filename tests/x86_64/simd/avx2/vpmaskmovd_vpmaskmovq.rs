...
fn test_vpmaskmovd_load_ymm6_ymm7_mem() {
    ...
    let (mut vcpu, mem) = setup_vm(&full_code, None);
    let data = vec![
        0xAA, 0xAA, 0xAA, 0xAA, 0xBB, 0xBB, 0xBB, 0xBB, 0xCC, 0xCC, 0xCC, 0xCC, 0xDD, 0xDD, 0xDD,
        0xDD, 0xEE, 0xEE, 0xEE, 0xEE, 0xFF, 0xFF, 0xFF, 0xFF, 0x11, 0x11, 0x11, 0x11, 0x22, 0x22,
        0x22, 0x22,
    ];
    mem.write_slice(&data, GuestAddress(ALIGNED_ADDR)).unwrap();
    run_until_hlt(&mut vcpu).unwrap();
...
fn test_vpmaskmovd_load_store_chain() {
    ...
    mem.write_slice(&zero_data, GuestAddress(ALIGNED_ADDR2))
        .unwrap();
    run_until_hlt(&mut vcpu).unwrap();
}
